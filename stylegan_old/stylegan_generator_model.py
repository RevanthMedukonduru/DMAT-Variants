...
class SynthesisModule(nn.Module):
  ...
  #   print("lod:", lod)
  #   print("Channels:", self.channels, "len: ", len(self.channels))
  #   x = self.layer0(w[:, 0])
  #   for block_idx in range(1, len(self.channels)):
  #     if block_idx + lod < len(self.channels):
  #       layer_idx = 2 * block_idx - 2
  #       print("w tensor shape:", w.shape)
  #       print("Attempting to access layer index:", layer_idx)
  #       if layer_idx == 0:
  ...
  def forward(self, w):
    ...
    x = self.layer0(w[:, 0])
    for block_idx in range(1, len(self.channels)):
        # Ensure block_idx is adjusted based on lod and doesn't exceed available layers in w
        if block_idx <= len(self.channels) - lod - 1:
            layer_idx = 2 * block_idx - 2
            # print("w tensor shape:", w.shape)
            # print("Attempting to access layer index:", layer_idx)
            # Ensure layer_idx does not exceed the size of w
            if layer_idx < w.shape[1]:
                if layer_idx == 0:
                    x = self.__getattr__(f'layer{layer_idx}')(w[:, layer_idx])
                else:
                    x = self.__getattr__(f'layer{layer_idx}')(x, w[:, layer_idx])
                layer_idx += 1  # Increment to get the next layer index within the block
                # Check again to ensure we don't exceed w's layer count
                if layer_idx < w.shape[1]:
                    x = self.__getattr__(f'layer{layer_idx}')(x, w[:, layer_idx])
                    image = self.__getattr__(f'output{block_idx - 1}')(x)
                else:
                    break  # Exit loop if next layer index is out of bounds
            else:
