...
parser = argparse.ArgumentParser(description="Training using ImageNet-10 - SINGLE NET - ADV")
parser.add_argument("--config", default="our_experiments/classifiers/single_net/adv/adv_config.yml")
...
start_epoch = 0
best_train_acc_c, best_train_loss_c, best_train_acc_a, best_train_loss_a = 0.0, 0.0, 0.0, 0.0
best_test_acc_c, best_test_loss_c, best_test_acc_a, best_test_loss_a = 0.0, 0.0, 0.0, 0.0
best_train_epoch_c, best_train_epoch_a = 0, 0
best_test_epoch_c, best_test_epoch_a = 0, 0
if args.resume:
...
def train_adv(epoch):
    ...
    for batch_idx, (images, _, labels) in enumerate(progress_bar):
        ...
        overall_loss = loss_a # only adversarial cross entropy loss is considered. (But if wanted to balance b/w clean, adv: 0.5*(loss_c + loss_a))
        ...
        if is_ig:
            ...
            # IG Loss
            ig_loss = torch.abs(entropy_c - entropy_a).mean(0)
        ...
        # Update progress bar
        if is_ig:
            progress_bar.set_description(f"E: [{epoch}] Train C.Loss: {loss_meter_c.avg:.4f}, C.Acc: {acc_meter_c.avg:.3f}, A.Loss: {loss_meter_a.avg:.4f}, A.Acc: {acc_meter_a.avg:.3f}, IG.Loss: {ig_loss_meter.avg:.4f}, O.Loss: {overall_loss_meter.avg:.4f}")
        else:
            progress_bar.set_description(f"E: [{epoch}] Train C.Loss: {loss_meter_c.avg:.4f}, C.Acc: {acc_meter_c.avg:.3f}, A.Loss: {loss_meter_a.avg:.4f}, A.Acc: {acc_meter_a.avg:.3f}")
            
    if is_ig:
        return loss_meter_c.avg, acc_meter_c.avg, loss_meter_a.avg, acc_meter_a.avg, ig_loss_meter.avg, overall_loss_meter.avg
    else:
        return loss_meter_c.avg, acc_meter_c.avg, loss_meter_a.avg, acc_meter_a.avg
...
def test_adv(epoch):
    ...
    for batch_idx, (images, _, labels) in enumerate(progress_bar):
        ...
        # Update progress bar
        progress_bar.set_description(f"E: [{epoch}] Test C.Loss: {loss_meter_c.avg:.4f}, C.Acc: {acc_meter_c.avg:.3f}, A.Loss: {loss_meter_a.avg:.4f}, A.Acc: {acc_meter_a.avg:.3f}")
...
for epoch in (range(start_epoch, cfg.num_epochs)):
    ...
    # Train  
    if is_ig:
        epoch_train_loss_c, epoch_train_acc_c, epoch_train_loss_a, epoch_train_acc_a, epoch_train_ig_loss, epoch_train_overall_loss = train_adv(epoch)
    else:
        epoch_train_loss_c, epoch_train_acc_c, epoch_train_loss_a, epoch_train_acc_a = train_adv(epoch)
    
    # Test
    epoch_test_loss_c, epoch_test_acc_c, epoch_test_loss_a, epoch_test_acc_a = test_adv(epoch)
    ...
    logging.info(f"Epoch: {epoch} | LR: {lr} | Train C.Loss: {epoch_train_loss_c}, C.Acc: {epoch_train_acc_c}, A.Loss: {epoch_train_loss_a}, A.Acc: {epoch_train_acc_a} | Test C.Loss: {epoch_test_loss_c}, C.Acc: {epoch_test_acc_c}, A.Loss: {epoch_test_loss_a}, A.Acc: {epoch_test_acc_a}")
...
print(f"Best | Test - C.Acc: {best_test_acc_c}, C.Loss: {best_test_loss_c}, A.Acc: {best_test_acc_a}, A.Loss: {best_test_loss_a} | Train - C.Acc: {best_train_acc_c}, C.Loss: {best_train_loss_c}, A.Acc: {best_train_acc_a}, A.Loss: {best_train_loss_a}")
