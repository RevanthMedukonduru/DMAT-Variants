...
def train_IG(epoch):
    ...
    for batch_idx, (images, latents, labels) in enumerate(progress_bar):
        ...
        if batch_idx % 50 == 0:
            ...
            save_image(images_ladv[:8], adv_latent_path, nrow=8, padding=2, normalize=True, value_range=(-1., 1.))
...
def train(epoch):
    ...
    for batch_idx, (images, latents, labels) in enumerate(progress_bar):
        images, latents, labels = images.to(device), latents.to(device), labels.to(device)
        print("Images Shape: ", images.shape, "Latents Shape: ", latents.shape, "Labels Shape: ", labels.shape)
